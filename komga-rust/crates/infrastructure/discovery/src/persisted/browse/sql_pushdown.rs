            ...
            match value {
                ...
                BookValueCondition::SeriesId(inc) => {
                    if empty_include(inc) {
                        ...
                        match inc {
                            InclusionCondition::Include(ids) => {
                                push_text_in_list(&mut e, "b.SERIES_ID", ids, false);
                            }
                            InclusionCondition::Exclude(ids) => {
                                e.push("NOT ");
                                push_text_in_list(&mut e, "b.SERIES_ID", ids, false);
...
/// True when `sql` references `alias.` with a non-identifier char before
/// the alias (space, `(`, string start) — so `(m.`/`COALESCE(m.` match
/// but `bm.`/`sm.` do not.
fn refs_alias(sql: &str, alias: &str) -> bool {
    let needle = format!("{alias}.");
    for (i, _) in sql.match_indices(&needle) {
        let ok = match i.checked_sub(1) {
            None => true,
            Some(j) => {
                let c = sql.as_bytes()[j] as char;
...
pub(super) async fn try_load_series_page(
    ...
    let count_from = {
        let mut f = "SERIES s".to_string();
        if refs_alias(&where_clause, "sm") {
            f.push_str(" LEFT JOIN SERIES_METADATA sm ON sm.SERIES_ID = s.ID");
        }
        if refs_alias(&where_clause, "bma") {
            f.push_str(" LEFT JOIN BOOK_METADATA_AGGREGATION bma ON bma.SERIES_ID = s.ID");
        }
        // rps/cs are ORDER-BY-only: WHERE conditions use subquery aliases
        f
    };
    let count_sql = format!("SELECT COUNT(*) AS COUNT FROM (SELECT s.ID FROM {count_from} WHERE {where_clause})");
    let total_elements = run_count(pool, &count_sql, &where_binds).await?;
...
/// Lightweight alphabetical groups: a single GROUP BY SQL that aggregates the
/// title-sort first character (Kotlin countByFirstCharacter equivalent) instead
/// of fetching every visible series ID + title sort. Keeps the in-memory
/// `first_group_key` semantics: trim, lowercase first char, empty -> '#'.
/// Returns None when the condition cannot be pushed down (caller falls back to
/// the in-memory path).
pub(super) async fn try_load_series_groups(
    backend: &SqliteDiscoveryBrowseService,
    context: &DiscoveryQueryContext,
    query: &PersistedSeriesBrowseQuery,
    ...
    // The group expression always references sm (title sort), so sm is always
    // joined; bma is only needed when a pushed-down condition references it.
    let mut from_body =
        "SERIES s LEFT JOIN SERIES_METADATA sm ON sm.SERIES_ID = s.ID".to_string();
    if refs_alias(&where_clause, "bma") {
        from_body.push_str(" LEFT JOIN BOOK_METADATA_AGGREGATION bma ON bma.SERIES_ID = s.ID");
    }

    const GROUP_EXPR: &str = "CASE WHEN TRIM(COALESCE(sm.TITLE_SORT, sm.TITLE, s.NAME)) = ''         THEN '#' ELSE lower(substr(TRIM(COALESCE(sm.TITLE_SORT, sm.TITLE, s.NAME)), 1, 1)) END";
    let group_sql = format!(
        "SELECT {GROUP_EXPR} AS GRP, COUNT(*) AS CNT FROM {from_body} WHERE {where_clause} GROUP BY GRP ORDER BY GRP"
    );
    let rows = bind_query(sqlx::query(sqlx::AssertSqlSafe(group_sql)), &where_binds)
        .fetch_all(backend.db.read_pool())
        .await?;
    Ok(Some(
        rows.into_iter()
            .map(|row| (row.get::<String, _>("GRP"), row.get::<i64, _>("CNT")))
            .collect(),
    ))
}
...
pub(super) async fn try_load_books_page(
    ...
    let count_from = {
        let mut f = "BOOK b JOIN SERIES s ON s.ID = b.SERIES_ID".to_string();
        if refs_alias(&where_clause, "bm") {
            f.push_str(" LEFT JOIN BOOK_METADATA bm ON bm.BOOK_ID = b.ID");
        }
        if refs_alias(&where_clause, "m") {
            f.push_str(" LEFT JOIN MEDIA m ON m.BOOK_ID = b.ID");
        }
        if refs_alias(&where_clause, "sm") {
            f.push_str(" LEFT JOIN SERIES_METADATA sm ON sm.SERIES_ID = s.ID");
        }
        // rp/rb are ORDER-BY-only: WHERE conditions use rp2/rb2 subquery aliases
        f
    };
    let count_sql = format!("SELECT COUNT(*) AS COUNT FROM (SELECT b.ID FROM {count_from} WHERE {where_clause})");
    let total_elements = run_count(pool, &count_sql, &where_binds).await?;
...
pub(super) async fn try_load_search_books_page(
    ...
    let total_elements = if sort_in_sql {
        let count_from = {
            let mut f = "BOOK b JOIN SERIES s ON s.ID = b.SERIES_ID".to_string();
            if refs_alias(&where_sql, "bm") {
                f.push_str(" LEFT JOIN BOOK_METADATA bm ON bm.BOOK_ID = b.ID");
            }
            if refs_alias(&where_sql, "m") {
                f.push_str(" LEFT JOIN MEDIA m ON m.BOOK_ID = b.ID");
            }
            if refs_alias(&where_sql, "sm") {
                f.push_str(" LEFT JOIN SERIES_METADATA sm ON sm.SERIES_ID = s.ID");
            }
            f
        };
        let count_sql = format!("SELECT COUNT(*) AS COUNT FROM (SELECT b.ID FROM {count_from} WHERE {where_sql})");
        run_count(pool, &count_sql, &where_binds).await?
    } else {
        0
    };
...
pub(super) async fn try_load_search_series_page(
    ...
    let total_elements = if sort_in_sql {
        let count_from = {
            ...
            if refs_alias(&where_sql, "bma") {
                f.push_str(" LEFT JOIN BOOK_METADATA_AGGREGATION bma ON bma.SERIES_ID = s.ID");
            }
            f
        };
        let count_sql = format!("SELECT COUNT(*) AS COUNT FROM (SELECT s.ID FROM {count_from} WHERE {where_sql})");
        run_count(pool, &count_sql, &where_binds).await?
    } else {
        0
    };
