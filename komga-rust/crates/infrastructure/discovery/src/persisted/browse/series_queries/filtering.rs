...
async fn enrich_series_read_progress(
    ...
    if let Some(user_id) = user_id {
        let read_progress = match ids {
            // Small IN batches beat a full-table scan; large sets (unpaged
            // full loads) keep the single full-table query.
            Some(ids) if ids.len() <= 500 => {
                backend.load_series_read_progress_counts_for_ids(user_id, ids).await?
            }
            _ => backend.load_series_read_progress_counts(user_id).await?,
        };
...
pub(in crate::persisted::browse) async fn load_persisted_series_page(
    ...
    // Enrich read progress counts on the paginated result (full fallback
    // path: content is already the full scan, keep the single table query).
    let content = enrich_series_read_progress(
        backend,
        context.user_id.as_deref(),
        page.content
