...
pub(crate) async fn load_persisted_book_detail(
    ...
    Ok(row.map(|row| book_read_model_from_row(&row)))
}
...
fn book_read_model_from_row(row: &sqlx::sqlite::SqliteRow) -> BookReadModel {
    BookReadModel {
...
pub(crate) async fn load_persisted_book_details_for_ids(
    pool: &SqlitePool,
    book_ids: &[String],
    user_id: Option<&str>,
    ...
    for chunk in book_ids.chunks(500) {
        ...
        separated.push_unseparated(") AND (rp.USER_ID = ");
        drop(separated);
        query.push_bind(user_id.unwrap_or_default());
        let mut tail = query.separated("");
        tail.push_unseparated(" OR rp.USER_ID IS NULL)");
        let rows = query
            .build()
            .fetch_all(pool)
            .await
            .context("query persisted book details batch")?;
        for row in rows {
            let model = book_read_model_from_row(&row);
            results.push((model.id.clone(), model));
        }
    }
    Ok(results)
}

pub(crate) async fn load_persisted_book_resources_for_ids(
    pool: &SqlitePool,
    book_ids: &[String],
    ...
    for chunk in book_ids.chunks(500) {
        let mut query = sqlx::QueryBuilder::<sqlx::Sqlite>::new(
            r#"SELECT b.ID AS ID, b.LIBRARY_ID, sm.AGE_RATING,
                COALESCE((SELECT GROUP_CONCAT(LABEL, char(30))
                          FROM (SELECT sms.LABEL AS LABEL
                                FROM SERIES_METADATA_SHARING sms
                                WHERE sms.SERIES_ID = s.ID
        ...
        separated.push_unseparated(") GROUP BY b.ID, b.LIBRARY_ID, sm.AGE_RATING");
        let rows = query.build().fetch_all(pool).await.context("query persisted book resources batch")?;
        for row in rows {
            results.push((
                row.get::<String, _>("ID"),
