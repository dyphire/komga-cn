...
use komga_infrastructure_base::stored_paths::{resolve_rooted_path, resolve_stored_path};
...
async fn persist_scanned_library_inner(
    ...
    let outcome: PersistScannedLibraryOutcome = 'outcome: {
        ...
        let library_row = sqlx::query(
            r#"SELECT UNAVAILABLE_DATE, ROOT
FROM LIBRARY
WHERE ID = ?
LIMIT 1"#,
        )
        ...
        let library_was_unavailable = library_row
            ...
            .is_some();
        let library_root = resolve_stored_path(&library_row.get::<String, _>("ROOT"));
...
WHERE LIBRARY_ID = ?
  AND DELETED_DATE IS NULL"#,
    ...
    // Stored URLs may be absolute filesystem paths, file: URLs, or relative
    // paths (FilesystemBookImport strips the library root). Resolve each URL
    // against the library root so relative URLs cannot bypass the protection
    // and get soft-deleted after a failed directory read.
    let is_protected = |url: &str| {
        let url_path = resolve_rooted_path(library_root, url);
        failed_directories
            .iter()
