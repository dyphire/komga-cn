    ...
    R: RememberMeRuntime + ?Sized,
{
    ...
    let Some(token) = remember_me_token.and_then(non_empty_token) else {
        return Ok(None);
    };

    Ok(remember_me_runtime
        .resolve_remember_me_user(token)?
...
mod tests {
    ...
    fn resolve_authenticated_user_checks_session_before_remember_me() {
        ...
        .expect("session resolution should not fail")
        ...
        .expect("remember-me resolution should not fail")
