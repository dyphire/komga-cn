...
use super::LegacyKeyedConditionEntry;
use super::filter_values::{parse_read_status_value, parse_u16_value};
...
fn parse_single_series_value_condition(
    ...
    match condition_type {
        ...
        "ReadStatus" => {
            let value = parse_string_value(condition, "value").unwrap_or_default();
        ...
        "SeriesStatus" => {
            let value = parse_string_value(condition, "value")
                ...
                .trim()
                .to_string();
            let Some(status) = SeriesStatus::parse(&value) else {
                return Err(DiscoveryError::InvalidSemantics(
                    "SeriesStatus filter requires a valid value".to_string(),
                ));
            };
            Ok(SeriesValueCondition::SeriesStatus(
                match operator.as_str() {
                    "isnot" => SeriesStatusCondition::Exclude(vec![status]),
                    "is" => SeriesStatusCondition::Include(vec![status]),
                    _ => {
                        return Err(DiscoveryError::InvalidSemantics(format!(
...
fn parse_legacy_keyed_series_condition(
    ...
    let condition_type = legacy_series_condition_type(entry.key)?;
...
mod tests {
    ...
    #[test]
    fn legacy_keyed_filter_parses_inside_series_filter_boundary() {
    ...
    #[test]
    fn legacy_collection_number_sort_requires_collection_scope() {
        let sorts = vec!["collection.number,asc".to_string()];

        assert!(parse_legacy_series_sorts(&sorts, None, None).is_empty());
        assert_eq!(
            parse_legacy_series_sorts(&sorts, None, Some(&vec!["collection-1".to_string()])),
            vec![SeriesSort::CollectionNumberAsc],
        );
    }
}
