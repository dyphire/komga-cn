...
use komga_application::media_assets::{
    BookMediaContentPort, BookMediaReaderPort, content_type_from_filename,
};
use komga_application::opds::OpdsBookFeedEntry;
...
use super::super::feeds::{
    OpdsV1AcquisitionEntry, OpdsV1XmlLink, normalize_opds_updated, query_escape,
};
use super::super::types::PersistedBookFeedItem;

struct OpdsPageStreamingLinkTarget {
    media_type: String,
    href: String,
}
...
async fn opds_book_page_streaming_links(
    ...
    let link_target =
        if media_types.len() == 1 && supported_formats.contains(&media_types[0].as_str()) {
            OpdsPageStreamingLinkTarget {
                media_type: media_types[0].clone(),
    ...
    let mut link = OpdsV1XmlLink::new(
        link_target.media_type,
...
async fn opds_book_page_stream_media_types(
    ...
    if media_type == "application/pdf" {
        return Ok(vec!["image/jpeg".to_string()]);
    }
...
async fn load_divina_page_media_types_for_opds(
    ...
    if !persisted.is_empty() {
        return Ok(dedup_media_types(persisted));
    }
    ...
    if media_content_type.starts_with("image/") {
        return Ok(vec![media_content_type]);
    }

    let archive_pages = content.archive_page_rows(&media).await?.unwrap_or_default();
    Ok(dedup_media_types(
        archive_pages
            .into_iter()
            .map(|page| {
                if page.media_type.is_empty() {
