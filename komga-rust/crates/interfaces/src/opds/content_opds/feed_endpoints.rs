...
pub(super) async fn opds_v2_on_deck_feed(
    ...
    let books = match app
        .services
        .opds_catalog
        .load_on_deck_books(user_id.clone(), library_id.map(str::to_string))
        .await
    {
        Ok(books) => books,
        Err(error) => {
...
async fn load_visible_latest_books_page(
    ...
    loop {
        let batch = app
            .services
            .opds_catalog
            .load_latest_books_paged(allowed_library_ids.clone(), None, None, offset, scan_limit)
            .await?;
        if batch.is_empty() {
            break;
        }
...
pub(super) async fn opds_v2_readlists_feed(
    ...
    let readlists = match match library_id {
        ...
        None => load_all_readlists(app.services.opds_catalog.as_ref()).await,
    } {
    ...
    let visible_readlists = if library_id.is_some() {
        ...
        for readlist in readlists {
            let readlist_books =
                match load_readlist_books(app.services.opds_persisted.as_ref(), &readlist.id).await
                {
                    Ok(books) => books,
                    Err(_) => continue,
                };
