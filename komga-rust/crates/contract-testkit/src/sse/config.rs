...
impl SseHarnessConfig {
    pub fn load_default() -> anyhow::Result<Self> {
        bail!("default SSE contract cases were removed; use SseHarnessConfig::load_from(path)")
    }

    pub fn load_from(path: impl Into<PathBuf>) -> anyhow::Result<Self> {
        let path = path.into();
        let content = fs::read_to_string(&path).with_context(|| {
            format!("failed to read SSE contract cases from {}", path.display())
        })?;
        toml::from_str(&content)
            .with_context(|| format!("failed to parse SSE contract cases from {}", path.display()))
    }
}
