...
async fn router_opds_v1_ondeck_returns_atom_feed() {
    let ctx = TestFixture::new("router-opds-v1-ondeck-feed").await;
...
async fn router_opds_v1_ondeck_uses_kotlin_acquisition_entry_shape() {
    let ctx = TestFixture::new("router-opds-v1-ondeck-entry-shape").await;
    seed_router_read_progress(ctx.paths(), true).await;
...
async fn router_opds_v1_ondeck_includes_page_streaming_link_without_read_progress_attributes() {
    let ctx = TestFixture::new("router-opds-v1-ondeck-pse-link").await;
    seed_router_read_progress(ctx.paths(), true).await;
    seed_router_pdf_book(
        ctx.paths(),
        "book-pdf",
...
async fn router_opds_v1_ondeck_orders_series_by_most_recent_read_date() {
    let ctx = TestFixture::new("router-opds-v1-ondeck-recent-order").await;
    seed_router_contract_nullable_samples(ctx.paths()).await;
    seed_router_read_progress(ctx.paths(), true).await;
    seed_router_pdf_book(
        ctx.paths(),
        "book-pdf-1",
...
async fn router_opds_v1_keep_reading_filters_non_ready_books() {
    let ctx = TestFixture::new("router-opds-v1-keep-reading-ready-only").await;
...
async fn router_opds_v1_keep_reading_uses_kotlin_acquisition_entry_shape() {
    let ctx = TestFixture::new("router-opds-v1-keep-reading-entry-shape").await;
    seed_router_read_progress(ctx.paths(), false).await;
    seed_router_pdf_book(
        ctx.paths(),
        "book-pdf",
...
async fn router_opds_v1_keep_reading_includes_page_streaming_link_with_read_progress_attributes() {
    let ctx = TestFixture::new("router-opds-v1-keep-reading-pse-link").await;
    seed_router_read_progress(ctx.paths(), false).await;
    seed_router_pdf_book(
        ctx.paths(),
        "book-pdf",
...
async fn router_opds_v1_keep_reading_preserves_zero_page_count_in_pse_link() {
    let ctx = TestFixture::new("router-opds-v1-keep-reading-zero-page-count").await;
    seed_router_read_progress(ctx.paths(), false).await;
    seed_router_pdf_book(
        ctx.paths(),
        "book-pdf",
...
async fn router_opds_v1_keep_reading_orders_by_read_progress_date() {
    let ctx = TestFixture::new("router-opds-v1-keep-reading-order").await;
    seed_router_read_progress(ctx.paths(), false).await;
    seed_router_pdf_book(
        ctx.paths(),
        "book-pdf",
...
async fn router_opds_v1_books_latest_uses_kotlin_acquisition_entry_shape() {
    let ctx = TestFixture::new("router-opds-v1-books-latest-entry-shape").await;
    let auth_token = ctx.login_admin().await;
...
async fn router_opds_v1_books_latest_filters_non_ready_books() {
    let ctx = TestFixture::new("router-opds-v1-books-latest-ready-only").await;
    seed_router_pdf_book(
        ctx.paths(),
        "book-pdf",
        "series-1",
...
async fn router_opds_v1_books_latest_paginates_after_restriction_filtering() {
    let ctx = TestFixture::new("router-opds-v1-books-latest-restriction-pagination").await;
    seed_router_contract_nullable_samples(ctx.paths()).await;
    seed_router_custom_series(ctx.paths(), "series-visible", "Visible Series", "library-1").await;
    seed_router_pdf_book(
        ctx.paths(),
        "book-visible",
        "series-visible",
    ...
    sqlx::query("UPDATE BOOK SET CREATED_DATE = ? WHERE ID = ?")
        ...
        .execute(&pool)
        .await
        .expect("book-visible created date should update for restriction-pagination test");
    pool.close().await;
...
async fn router_opds_v1_books_latest_includes_page_streaming_and_read_progress_for_pdf_books() {
    let ctx = TestFixture::new("router-opds-v1-books-latest-pse-read-progress").await;
    seed_router_pdf_book(
        ctx.paths(),
        "book-pdf",
        "series-1",
