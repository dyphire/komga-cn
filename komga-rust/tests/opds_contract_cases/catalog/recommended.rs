...
async fn router_opds_v2_library_recommended_unauthorized_returns_opds_auth_document() {
    let ctx = TestFixture::new("router-opds-v2-library-recommended-unauthorized-auth-doc").await;
...
async fn router_opds_v2_library_recommended_uses_kotlin_shape() {
    let ctx = TestFixture::new("router-opds-v2-library-recommended-shape").await;
