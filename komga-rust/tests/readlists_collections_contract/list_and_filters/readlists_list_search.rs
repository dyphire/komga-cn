...
#[tokio::test]
async fn router_readlists_search_matches_ten_char_intra_token_name_ngrams_like_kotlin() {
    let paths = new_router_fixture("router-readlists-search-ten-char-ngram").await;
    seed_router_contract_data(&paths).await;

    let pool = connect_pool(paths.main_db.as_path(), 1)
        .await
        .expect("main db should open for readlists ten-char ngram seed");
    sqlx::query("UPDATE READLIST SET NAME = ?, SUMMARY = ? WHERE ID = ?")
        .bind("Encyclopedia ReadList")
        .bind("")
        .bind("readlist-1")
        .execute(&pool)
        .await
        .expect("readlist-1 should update for readlists ten-char ngram seed");
    pool.close().await;
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/readlists?search=cyclopedia&unpaged=true")
                .header("x-auth-token", &auth_token)
                .body(Body::empty())
                .expect("readlists ten-char ngram search request should build"),
        )
        .await
        .expect("readlists ten-char ngram search request should complete");
    ...
    let ids = payload
        .get("content")
        .and_then(Value::as_array)
        .expect("readlists ten-char ngram payload should expose content array")
        .iter()
        .map(|entry| {
            entry
                .get("id")
                .and_then(Value::as_str)
                .expect("readlists ten-char ngram entry should expose id")
                .to_string()
        })
        .collect::<Vec<_>>();
    assert_eq!(ids, vec!["readlist-1"]);
...
#[tokio::test]
async fn router_readlists_search_matches_long_single_token_names_like_kotlin() {
    let paths = new_router_fixture("router-readlists-search-long-single-token").await;
    seed_router_contract_data(&paths).await;

    let long_token = "supercalifragilisticexpialidociousencyclopedia";
    let pool = connect_pool(paths.main_db.as_path(), 1)
        .await
        .expect("main db should open for readlists long single-token seed");
    sqlx::query("UPDATE READLIST SET NAME = ?, SUMMARY = ? WHERE ID = ?")
        .bind(long_token)
        .bind("")
        .bind("readlist-1")
        .execute(&pool)
        .await
        .expect("readlist-1 should update for readlists long single-token seed");
    pool.close().await;
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(&format!(
                    "/api/v1/readlists?search={long_token}&unpaged=true"
                ))
                .header("x-auth-token", &auth_token)
                .body(Body::empty())
                .expect("readlists long single-token search request should build"),
        )
        .await
        .expect("readlists long single-token search request should complete");
    ...
    let ids = payload
        .get("content")
        .and_then(Value::as_array)
        .expect("readlists long single-token payload should expose content array")
        .iter()
        .map(|entry| {
            entry
                .get("id")
                .and_then(Value::as_str)
                .expect("readlists long single-token entry should expose id")
                .to_string()
        })
        .collect::<Vec<_>>();
    assert_eq!(ids, vec!["readlist-1"]);
...
#[tokio::test]
async fn router_readlists_search_preserves_backend_relevance_order_without_name_tiebreak() {
    let paths = new_router_fixture("router-readlists-search-backend-rank-order").await;
    seed_router_contract_data(&paths).await;

    let pool = connect_pool(paths.main_db.as_path(), 1)
        .await
        .expect("main db should open for readlists backend-rank order seed");
    sqlx::query("UPDATE READLIST SET NAME = ?, SUMMARY = ? WHERE ID = ?")
        .bind("Zulu Alpha ReadList")
        .bind("")
        .bind("readlist-1")
        .execute(&pool)
        .await
        .expect("readlist-1 should update for readlists backend-rank order seed");
    sqlx::query("INSERT INTO READLIST (ID, NAME, SUMMARY, BOOK_COUNT) VALUES (?, ?, ?, ?)")
        .bind("readlist-2")
        .bind("Alpha Bravo ReadList")
        .bind("")
        .bind(1_i64)
        .execute(&pool)
        .await
        .expect("readlist-2 row should insert for readlists backend-rank order seed");
    sqlx::query("INSERT INTO READLIST_BOOK (READLIST_ID, BOOK_ID, NUMBER) VALUES (?, ?, ?)")
        .bind("readlist-2")
        .bind("book-1")
        .bind(0_i64)
        .execute(&pool)
        .await
        .expect("readlist-2 membership should insert for readlists backend-rank order seed");
    pool.close().await;
    ...
    let expected_ids = SearchIndexLifecycle::bootstrap(config.lucene_data_directory.as_path())
        .expect("readlists backend-rank order search index should bootstrap")
        .search_ids("alpha", SearchEntityType::ReadList, 1000)
        .expect("readlists backend-rank order search query should succeed");
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .header("x-auth-token", &auth_token)
                .body(Body::empty())
                .expect("readlists backend-rank order request should build"),
        )
        .await
        .expect("readlists backend-rank order request should complete");
    ...
    let ids = payload
        .get("content")
        .and_then(Value::as_array)
        .expect("readlists backend-rank order payload should expose content array")
        .iter()
        .map(|entry| {
            entry
                .get("id")
                .and_then(Value::as_str)
                .expect("readlists backend-rank order entry should expose id")
                .to_string()
        })
        .collect::<Vec<_>>();
...
#[tokio::test]
async fn router_readlists_duplicate_search_matches_current_kotlin_contract() {
    let paths = new_router_fixture("router-readlists-duplicate-search-contract").await;
    seed_router_contract_data(&paths).await;

    let pool = connect_pool(paths.main_db.as_path(), 1)
        .await
        .expect("main db should open for readlists duplicate search seed");
    sqlx::query("UPDATE READLIST SET NAME = ?, SUMMARY = ? WHERE ID = ?")
        .bind("Lib1")
        .bind("")
        .bind("readlist-1")
        .execute(&pool)
        .await
        .expect("readlist-1 should update for readlists duplicate search seed");
    sqlx::query("INSERT INTO READLIST (ID, NAME, SUMMARY, BOOK_COUNT) VALUES (?, ?, ?, ?)")
        .bind("readlist-2")
        .bind("Lib2")
        .bind("")
        ...
        .execute(&pool)
        .await
        .expect("readlist-2 row should insert for readlists duplicate search seed");
    sqlx::query("INSERT INTO READLIST_BOOK (READLIST_ID, BOOK_ID, NUMBER) VALUES (?, ?, ?)")
        .bind("readlist-2")
        .bind("book-1")
        .bind(0_i64)
        .execute(&pool)
        .await
        .expect("readlist-2 membership should insert for readlists duplicate search seed");
    sqlx::query("INSERT INTO READLIST (ID, NAME, SUMMARY, BOOK_COUNT) VALUES (?, ?, ?, ?)")
        .bind("readlist-3")
        .bind("Lib1+2")
        .bind("")
        ...
        .execute(&pool)
        .await
        .expect("readlist-3 row should insert for readlists duplicate search seed");
    sqlx::query("INSERT INTO READLIST_BOOK (READLIST_ID, BOOK_ID, NUMBER) VALUES (?, ?, ?)")
        .bind("readlist-3")
        .bind("book-1")
        .bind(0_i64)
        .execute(&pool)
        .await
        .expect("readlist-3 membership should insert for readlists duplicate search seed");
    pool.close().await;
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/api/v1/readlists?search=Lib1&search=Lib2")
                .header("x-auth-token", &auth_token)
                .body(Body::empty())
                .expect("readlists duplicate search request should build"),
        )
        .await
        .expect("readlists duplicate search request should complete");
    ...
    let content = payload
        .get("content")
        .and_then(Value::as_array)
        .expect("readlists duplicate search payload should expose content array");
    ...
    assert!(
        content
            .iter()
            .any(|entry| entry.get("name") == Some(&json!("Lib1")))
    );
    assert!(
        content
            .iter()
            .any(|entry| entry.get("name") == Some(&json!("Lib2")))
    );
    assert!(
        content
            .iter()
            .any(|entry| entry.get("name") == Some(&json!("Lib1+2")))
    );
