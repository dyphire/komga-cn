use crate::compat::ComparisonMode;
use anyhow::Context;
...
impl HarnessConfig {
    pub fn load_default() -> anyhow::Result<Self> {
        Self::load_from(Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/compat/cases.toml"))
    }

    pub fn load_from(path: impl Into<PathBuf>) -> anyhow::Result<Self> {
        let path = path.into();
        let content = fs::read_to_string(&path)
            .with_context(|| format!("failed to read compat cases from {}", path.display()))?;
        toml::from_str(&content)
            .with_context(|| format!("failed to parse compat cases from {}", path.display()))
    }
}
