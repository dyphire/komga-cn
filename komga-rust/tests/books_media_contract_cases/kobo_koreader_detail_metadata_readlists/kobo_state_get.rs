...
#[tokio::test]
async fn router_kobo_state_returns_internal_error_when_book_lookup_fails() {
    let ctx = TestFixture::builder("router-kobo-state-existence-check-error")
    ...
    let pool = connect_test_pool(ctx.paths().main_db.as_path(), 1)
        .await
        .expect("main db should open for kobo state created timestamp failure setup");
    sqlx::query("ALTER TABLE BOOK RENAME COLUMN CREATED_DATE TO CREATED_DATE_BROKEN")
        .execute(&pool)
        .await
        .expect(
            "book created date column should be renamed for kobo state timestamp failure setup",
        );
    pool.close().await;

    let timestamp_response = ctx
        .app()
        .clone()
        .oneshot(
            Request::builder()
                ...
                .body(Body::empty())
                .expect("kobo state created timestamp failure GET request should build"),
        )
        .await
        .expect("kobo state created timestamp failure GET request should complete");

    assert_eq!(
        timestamp_response.status(),
        StatusCode::INTERNAL_SERVER_ERROR
    );

    let pool = connect_test_pool(ctx.paths().main_db.as_path(), 1)
        .await
        .expect("main db should open for kobo state existence failure setup");
    sqlx::query("ALTER TABLE BOOK RENAME TO BOOK_BROKEN")
        .execute(&pool)
        .await
        .expect("book table should be renamed for kobo state existence failure setup");
    pool.close().await;
    ...
    let get_response = ctx
        ...
        .oneshot(
            Request::builder()
                ...
                .header("x-auth-token", &auth_token)
                .body(Body::empty())
                .expect("kobo state existence failure GET request should build"),
        )
        .await
        .expect("kobo state existence failure GET request should complete");
                    ...
                    json!({
                        "ReadingStates": [{
                            ...
                            "CurrentBookmark": {
                                ...
                                "Location": {"Source": "/book-1.xhtml#frag", "Value": "kobo.5.1"}
                            }
                        }]
                    })
                    .to_string(),
                ))
                .expect("kobo state existence failure PUT request should build"),
        )
        .await
        .expect("kobo state existence failure PUT request should complete");
