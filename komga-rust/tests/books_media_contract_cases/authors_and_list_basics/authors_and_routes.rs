...
async fn router_deprecated_authors_v1_route_matches_kotlin_shape() {
    let ctx = TestFixture::builder("router-deprecated-authors-v1-route")
...
async fn router_author_endpoints_filter_to_authorized_libraries() {
    let ctx = TestFixture::builder("router-authors-authorized-libraries")
        .with_seed(|paths| async move {
            seed_router_authors_scope_variants(&paths).await;
        })
        .build()
...
async fn router_author_endpoints_accept_basic_auth_like_kotlin_clients() {
    let ctx = TestFixture::builder("router-authors-basic-auth-compat")
        .with_seed(|paths| async move {
            seed_router_authors_scope_variants(&paths).await;
        })
        .build()
...
async fn router_authors_names_matches_search_without_accents() {
    let ctx = TestFixture::new("router-authors-names-strip-accents").await;
