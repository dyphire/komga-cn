...
pub(super) struct SingleResponseServer {
    pub(super) url: String,
    pub(super) join: tokio::task::JoinHandle<()>,
}

pub(super) fn kobo_proxy_env_lock() -> &'static Mutex<()> {
    static LOCK: OnceLock<Mutex<()>> = OnceLock::new();
    LOCK.get_or_init(|| Mutex::new(()))
}

pub(super) fn restore_env_var(name: &str, previous: Option<String>) {
    match previous {
...
pub(super) async fn spawn_single_response_server(
    status_code: u16,
    content_type: &str,
    body: &str,
...
pub(super) async fn upsert_server_setting(paths: &RuntimeDbPaths, key: &str, value: &str) {
...
pub(super) async fn seed_kobo_sync_api_key(paths: &RuntimeDbPaths, api_key: &str, user_id: &str) {
    let pool = connect_pool(paths.main_db.as_path(), 1)
        .await
...
pub(super) async fn seed_router_cbz_book(
