...
fn prepare_action_fixture(config_dir: &Path, email: &str, password: &str) {
    ...
    run_async(async {
        ...
        let main_pool = komga_rust::infrastructure::sqlite::connect_pool(&database_file, 1)
            .await
            .expect("CLI action main pool should open");
        komga_rust::infrastructure::sqlite::setup::bootstrap_pool(&main_pool)
            .await
            .expect("CLI action main schema should bootstrap");
        main_pool.close().await;

        let tasks_pool = komga_rust::infrastructure::sqlite::connect_pool(&tasks_db_file, 1)
            .await
            .expect("CLI action tasks pool should open");
        komga_rust::infrastructure::sqlite::setup::bootstrap_tasks_pool(&tasks_pool)
            .await
