...
#[tokio::test]
async fn router_opds_v1_book_page_does_not_negotiate_pdf_for_pdf_books() {
    let paths = new_router_fixture("router-opds-v1-book-page-no-pdf-negotiation").await;
    seed_router_contract_data(&paths).await;
    seed_router_pdf_book(
        &paths,
        "book-pdf-1",
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .body(Body::empty())
                .expect("opds v1 pdf negotiation request should build"),
        )
        .await
        .expect("opds v1 pdf negotiation request should complete");
    ...
    let content_type = response
        ...
        .and_then(|value| value.to_str().ok())
        .expect("opds v1 page should expose content-type");
    assert!(
...
#[tokio::test]
async fn router_opds_v2_book_page_does_not_negotiate_pdf_for_pdf_books() {
    let paths = new_router_fixture("router-opds-v2-book-page-no-pdf-negotiation").await;
    seed_router_contract_data(&paths).await;
    seed_router_pdf_book(
        &paths,
        "book-pdf-1",
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/opds/v2/books/book-pdf-1/pages/1")
                .header("x-auth-token", &auth_token)
                .header(header::ACCEPT, "application/pdf")
                .body(Body::empty())
                .expect("opds v2 pdf negotiation request should build"),
        )
        .await
        .expect("opds v2 pdf negotiation request should complete");
    ...
    let content_type = response
        ...
        .get(header::CONTENT_TYPE)
        .and_then(|value| value.to_str().ok())
        .expect("opds v2 page should expose content-type");
    assert!(
        content_type.starts_with("image/"),
        "content-type was: {content_type}"
    );
