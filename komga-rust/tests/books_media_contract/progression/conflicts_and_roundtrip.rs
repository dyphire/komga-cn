...
#[tokio::test]
async fn router_book_progression_get_returns_full_r2_progression_shape_on_opds_v2_route() {
    let paths = new_router_fixture("router-book-progression-opds-v2-get-full-shape").await;
    seed_router_contract_data(&paths).await;

    let pool = connect_pool(paths.main_db.as_path(), 1)
        .await
        .expect("main db should open for opds progression get seed");
    ...
    .bind("reader-9")
    .bind("Kobo Libra")
    .bind(serde_json::to_vec(&progression).expect("opds progression locator should serialize"))
    .execute(&pool)
    .await
    .expect("opds progression row should insert");
    pool.close().await;
